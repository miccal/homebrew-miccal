...
* `m-microsoft-teams-canary` and `m-microsoft-teams-continuous-deployment` can be used to install the latest versions of `microsoft-teams` from Microsoft's latest internal builds. These are used mainly for `livecheck` comparisons with the main `microsoft-teams` Cask.
